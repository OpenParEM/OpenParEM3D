...
void init_project (struct projectData *data) {
   ...
   data->mesh_refinement_port_limit=-40;
   ...
   data->refinement_variable=allocCopyString("invalid");    // This is an OpenParEM2D project variable.
   data->refinement_iteration_min=1;
   data->refinement_iteration_max=10;
   data->refinement_required_passes=3;
   data->refinement_tolerance=1e-3;
   ...
   data->touchstone_version=allocCopyString("1.1");
   ...
   data->solution_3D_tolerance=1e-4;
...
void free_project (struct projectData *data) {
   ...
   if (data->touchstone_version) free (data->touchstone_version);
...
void print_project (struct projectData *data, struct projectData *defaultData, const char *indent) {
   ...
   PetscPrintf(PETSC_COMM_WORLD,"%s%sproject.calculate.poynting %s\n",indent,comment[matched],logic[data->project_calculate_poynting]);
   ...
   PetscPrintf(PETSC_COMM_WORLD,"%s%smesh.save.refined %s\n",indent,comment[matched],logic[data->mesh_save_refined]);

   matched=0; if (defaultData && double_compare(data->mesh_refinement_port_limit,defaultData->mesh_refinement_port_limit,1e-14)) matched=1;
   PetscPrintf(PETSC_COMM_WORLD,"%s%smesh.refinement.port.limit %g\n",indent,comment[matched],data->mesh_refinement_port_limit);
   ...
   PetscPrintf(PETSC_COMM_WORLD,"%s%smesh.quality.limit %g\n",indent,comment[matched],data->mesh_quality_limit);
   ...
   matched=0; if (defaultData && double_compare(data->refinement_tolerance,defaultData->refinement_tolerance,1e-14)) matched=1;
   PetscPrintf(PETSC_COMM_WORLD,"%s%srefinement.tolerance %g\n",indent,comment[matched],data->refinement_tolerance);
   ...
   PetscPrintf(PETSC_COMM_WORLD,"%smaterials.default.boundary %s\n",indent,data->materials_default_boundary);
   ...
   PetscPrintf(PETSC_COMM_WORLD,"%s%sreference.impedance %g\n",indent,comment[matched],data->reference_impedance);

   matched=0; if (defaultData && strcmp(data->touchstone_version,defaultData->touchstone_version) == 0) matched=1;
   PetscPrintf(PETSC_COMM_WORLD,"%s%stouchstone.version %s\n",indent,comment[matched],data->touchstone_version);

   matched=0; if (defaultData && strcmp(data->touchstone_frequency_unit,defaultData->touchstone_frequency_unit) == 0) matched=1;
   PetscPrintf(PETSC_COMM_WORLD,"%s%stouchstone.frequency.unit %s\n",indent,comment[matched],data->touchstone_frequency_unit);

   matched=0; if (defaultData && strcmp(data->touchstone_format,defaultData->touchstone_format) == 0) matched=1;
   PetscPrintf(PETSC_COMM_WORLD,"%s%stouchstone.format %s\n",indent,comment[matched],data->touchstone_format);
   ...
   PetscPrintf(PETSC_COMM_WORLD,"%s%sdebug.show.port.definitions %s\n",indent,comment[matched],logic[data->debug_show_port_definitions]);
   ...
   PetscPrintf(PETSC_COMM_WORLD,"%s%sdebug.save.port.fields %s\n",indent,comment[matched],logic[data->debug_save_port_fields]);
...
void print_invalid_entry (PetscErrorCode *error, int lineNumber, const char *indent) {
   PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3133: Invalid entry at line %d.\n",indent,indent,lineNumber);
   *error=1;
   return;
}
...
PetscErrorCode load_project_file (const char *filename, struct projectData *data, const char* indent) {
   ...
   if (rank == 0) {
      ...
      if (fp) {
                  ...
                  if (strcmp(keyword,data->version_name) == 0) {
                     ...
                     if (is_text(value)) {
                        if (strcmp(value,data->version_value) != 0) {
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3134: Version mismatch. Expecting on first line: %s %s\n",
                     ...
                     PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3135: Missing version. Expecting on first line: %s %s\n",
                                 indent,indent,data->version_name,data->version_value);
                     ierr=1;
                  }
               }
         ...
         while (line_size >= 0) {
            ...
            if (! is_blank(line) && len > 0) {
               ...
               if (keyword != NULL) {
                  ...
                  if (strcmp(keyword,"project.calculate.poynting") == 0) {
                     ...
                     if (is_int(value)) {
                        ...
                        if (data->mesh_order < 1) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3136: Value must be >= 1 at line %d.\n",indent,indent,lineCount);
                        }
                        if (data->mesh_order > 20) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3137: Value must be <= 20 at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                  ...
                  else if (strcmp(keyword,"mesh.refinement.port.limit") == 0) {
                     value=strtok(NULL," ");
                     if (is_double(value)) {
                        data->mesh_refinement_port_limit=atof(value);
                        value=strtok(NULL," ");
                        if (is_text(value)) print_invalid_entry (&ierr,lineCount,indent);
                        if (data->mesh_refinement_port_limit >= 0) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3138: Value must be < 0 at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_double(value)) {
                        ...
                        if (data->mesh_3D_refinement_fraction <= 0) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3143: Value must be > 0 at line %d.\n",indent,indent,lineCount);
                        }
                        if (data->mesh_3D_refinement_fraction > 1) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3144: Value must be <= 1 at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_double(value)) {
                        ...
                        if (data->mesh_quality_limit <= 0) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3145: Value must be > 0 at line %d.\n",indent,indent,lineCount);
                        }
                        if (data->mesh_quality_limit > 1) {
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sWarning: Value at line %d is large.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_text(value)) {
                        ...
                        data->refinement_frequency=allocCopyString("highlow");
                        ierr=1;
                        PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3146: Value must be \"all\", \"none\", \"high\", \"low\", \"highlow (default)\", \"lowhigh\", or \"plan\" at line %d.\n",
                                indent,indent,lineCount);
                     }
                     if (! is_refinement_frequency(data->refinement_frequency)) {
                        ierr=1;
                        PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3147: Value must be \"all\", \"none\", \"high\", \"low\", \"highlow (default)\", \"lowhigh\", or \"plan\" at line %d.\n",
                                indent,indent,lineCount);
                     }
                  }
                     ...
                     if (is_int(value)) {
                        ...
                        if (data->refinement_required_passes < 1) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3148: Value must be >= 1 at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     value=strtok(NULL," ");
                     if (is_double(value)) {
                        data->refinement_tolerance=atof(value);
                        value=strtok(NULL," ");
                        if (is_text(value)) print_invalid_entry (&ierr,lineCount,indent);
                        if (data->refinement_tolerance <= 0) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3149: Value must be > 0 at line %d.\n",indent,indent,lineCount);
                        }
                        if (data->refinement_tolerance > 1) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3150: Value must be <= 1 at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_int(value)) {
                        ...
                        if (data->refinement_iteration_min < 1) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3151: Value must be >= 1 at line %d.\n",indent,indent,lineCount);
                        }
                        value=strtok(NULL," ");
                        if (is_text(value)) print_invalid_entry (&ierr,lineCount,indent);
                     } else print_invalid_entry (&ierr,lineCount,indent);
                     ...
                     if (is_double(value)) {
                        ...
                        if (data->reference_impedance < 0) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3153: Value must be >= 0 at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }

                  else if (strcmp(keyword,"touchstone.version") == 0) {
                     value=strtok(NULL," ");
                     if (is_text(value)) {
                        free(data->touchstone_version);
                        data->touchstone_version=allocCopyString(value);
                        value=strtok(NULL," ");
                        if (is_text(value)) print_invalid_entry (&ierr,lineCount,indent);
                        if (strcmp(data->touchstone_version,"1.0") != 0) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3154: Value must be either \"1.0\" or \"2.0\" at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_text(value)) {
                        ...
                        if (!(strcmp(data->touchstone_frequency_unit,"Hz") == 0) && 
                           ...
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3155: Value must be \"Hz\", \"kHz\", \"MHz\", or \"GHz\" at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_text(value)) {
                        ...
                        if (!(strcmp(data->touchstone_format,"DB") == 0) && 
                           ...
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3156: Value must be \"DB\", \"MA\", or \"RI\" at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_double(value)) {
                        ...
                        if (data->solution_temperature < -273.15) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3157: Value must be >= -273.15 at line %d.\n",indent,indent,lineCount);
                        } 
                        if (data->solution_temperature > 1e5) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3158: Value must be < 1e5 at line %d.\n",indent,indent,lineCount);
                        } 
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_double(value)) {
                        ...
                        if (data->solution_2D_tolerance <= 0) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3159: Value must be > 0 at line %d.\n",indent,indent,lineCount);
                        } 
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_double(value)) {
                        ...
                        if (data->solution_3D_tolerance <= 0) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3160: Value must be > 0 at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_int(value)) {
                        ...
                        if (data->solution_iteration_limit < 1) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3161: Value must be >= 1 at line %d.\n",indent,indent,lineCount);
                        } 
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_int(value)) {
                        ...
                        if (data->solution_modes_buffer < 0) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3162: Value must be >= 0 at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                     ...
                     if (is_double(value)) {
                        ...
                        if (data->solution_shift_factor < 1) {
                           ierr=1;
                           PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3163: Value must be >= 1 at line %d.\n",indent,indent,lineCount);
                        }
                     } else print_invalid_entry (&ierr,lineCount,indent);
                  }
                  ...
                  else {ierr=1; PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3164: Invalid entry at line %d.\n",indent,indent,lineCount);}
               }
            } 
         ...
         if (ierr) {PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3165: Failed to load project.\n",indent,indent); return ierr;}

         // some consistency checks

         if (data->inputFrequencyPlansCount == 0) {
            ierr=1;
            PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3166: At least one frequency plan is required.\n",indent,indent);
         } else {
            i=0;
            while (i < data->inputFrequencyPlansCount) {
               if (data->inputFrequencyPlans[i].type == 0 || data->inputFrequencyPlans[i].type == 1) {
                  if (data->inputFrequencyPlans[i].stop < data->inputFrequencyPlans[i].start) {
                     ierr=1;
                     PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3167: The stopping frequency must be >= to the starting frequency at line %d.\n",indent,indent,data->inputFrequencyPlans[i].lineNumber);
                  }
               }
               if (data->inputFrequencyPlans[i].type == 0 && data->inputFrequencyPlans[i].step <= 0) {
                  ierr=1;
                  PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3168: The frequency step must be positive at line %d.\n",indent,indent,data->inputFrequencyPlans[i].lineNumber);
               }
               if (data->inputFrequencyPlans[i].type == 1 && data->inputFrequencyPlans[i].pointsPerDecade < 1) {
                  ierr=1;
                  PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3169: The number of points per decade must be >= 1 at line %d.\n",indent,indent,data->inputFrequencyPlans[i].lineNumber);
               }
               i++;
            }
            ...
            if (strcmp(data->refinement_frequency,"plan") == 0 && ! has_refinementFrequencyPlan(data)) {
               ierr=1;
               PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3170: A refinement plan is called for at line %d but no plan is provided.\n",indent,indent,lineRefinementFrequency);
            }

            if (strcmp(data->refinement_frequency,"plan") != 0 && has_refinementFrequencyPlan(data)) {
               ierr=1;
               PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3171: A refinement plan is not called for at line %d but a refinement plan or plans is provided.\n",indent,indent,lineRefinementFrequency);
            }
         ...
         if (data->refinement_iteration_max < data->refinement_iteration_min) {
            ierr=1;//
            PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3172: The maximum iteration limit of %d must be >= to the minimum iteration limit of %d",indent,indent,data->refinement_iteration_max,data->refinement_iteration_min);
         ...
         PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3173: Failed to open file \"%s\" for reading.\n",indent,indent,filename);
         ierr=1;
      }

      if (ierr) PetscPrintf(PETSC_COMM_WORLD,"%s%sERROR3174: Failed to load project.\n",indent,indent);
      ...
      if (! ierr) {
         ...
         while (i < size) {
            ...
            ierr=MPI_Send(&(data->mesh_refinement_port_limit),1,MPI_DOUBLE,i,1000006,PETSC_COMM_WORLD);
            ...
            ierr=MPI_Send(&(data->refinement_tolerance),1,MPI_DOUBLE,i,1000031,PETSC_COMM_WORLD);
            ...
            length=strlen(data->touchstone_version);
            ierr=MPI_Send (&length,1,MPI_INT,i,1000042,PETSC_COMM_WORLD);
            ierr=MPI_Send(data->touchstone_version,length,MPI_CHAR,i,1000043,PETSC_COMM_WORLD);
      ...
      ierr=MPI_Recv(&(data->mesh_refinement_port_limit),1,MPI_DOUBLE,0,1000006,PETSC_COMM_WORLD,MPI_STATUS_IGNORE);
      ...
      ierr=MPI_Recv(&(data->refinement_tolerance),1,MPI_DOUBLE,0,1000031,PETSC_COMM_WORLD,MPI_STATUS_IGNORE);
      ...
      ierr=MPI_Recv(&length,1,MPI_INT,0,1000042,PETSC_COMM_WORLD,MPI_STATUS_IGNORE);
      if (data->touchstone_version) free(data->touchstone_version);
      data->touchstone_version=(char *) malloc((length+1)*sizeof(char));
      ierr=MPI_Recv(data->touchstone_version,length,MPI_CHAR,0,1000043,PETSC_COMM_WORLD,MPI_STATUS_IGNORE);
      data->touchstone_version[length]='\0';
